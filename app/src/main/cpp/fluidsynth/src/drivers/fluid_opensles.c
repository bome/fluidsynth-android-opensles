...
new_fluid_opensles_audio_driver2(fluid_settings_t* settings, fluid_audio_func_t func, void* data)
{
  ...
  else
  {
    ...
    {
        FLUID_MEMSET(dev->sles_buffer_short, 0, dev->period_frames * NUM_CHANNELS);
    }
  }
  ...
  FLUID_LOG(FLUID_INFO, "Using OpenSLES driver, period_size=%d samples.", dev->period_frames);
...
void opensles_callback(SLAndroidSimpleBufferQueueItf caller, void *pContext)
{
  ...
  //FLUID_LOG(FLUID_INFO, "opensles_callback: %d samples  is_float=%d", dev->period_frames, dev->is_sample_format_float);
...
fluid_opensles_audio_run(void* d)
{
  ...
  while (dev->cont)
  {
    //FLUID_LOG(FLUID_INFO, "fluid_opensles_audio_run: %d samples  is_float=%d", period_frames, dev->is_sample_format_float);
    adjust_latency (dev);
